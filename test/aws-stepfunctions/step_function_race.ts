...
import { InlineNodejsFunction, LateBoundStepFunctionsStartExecution, StepFunctionTask, StepFunctionTaskProps, Singleton } from "../../src";
...
export class StepFunctionRace extends StateMachine {
    constructor(scope: Construct, id: string, props?: StepFunctionRaceProps) {
        ...
        this.role.attachInlinePolicy(new Policy(this, "ExecutePolicy", { statements: [new PolicyStatement({
            actions: ["states:StartExecution"],
            effect: Effect.ALLOW,
            resources: ["*"]
        })]}))
...
export class StepFunctionRaceExecution extends Construct {
    constructor(scope: Construct, id: string, props: StepFunctionRaceExecutionProps) {
        ...
        let i = 1
        let inputs: any = props.contestants.map(c => { return {
    ...
    createReferee(props: StepFunctionRaceExecutionProps) {
        return Singleton.create(this, "StepFunctionRaceReferee",
            (s, id) => new RaceReferee(s, id)) as RaceReferee
    }

    createStateMachine(props: StepFunctionRaceExecutionProps) {
